...
const UserProfileForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log('auth logging:', auth.admin);
        const id = auth.adminId;
        const response = await axios.post(
          "https://finmap.onrender.com/api/v1/getAdmin",
          { adminId:id },
          {
            headers: {
  ...
  const renderProfileImage = () => {
    ...
    return profileImage ? `https://finmap.onrender.com/adminProfileImages/${profileImage}` : 'https://via.placeholder.com/150';
  };

  const handleImageUpdate = async () => {
    const formData = new FormData();
    formData.append('Picture', profileImage);
    formData.append('email', auth.admin);
    try {
      const response = await axios.put(
        'https://finmap.onrender.com/api/v1/Admin/updateDisplayPicture',
        formData,
        {
          headers: {
  ...
  const handleFormUpdate = async () => {
    try {
      const response = await axios.put(
        'https://finmap.onrender.com/api/v1/Admin/updateProfile',
        {
          name,
          phoneNo,
        },
