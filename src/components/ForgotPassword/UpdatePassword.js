...
const ResetPasswordPage = () => {
  ...
  const handleUpdatePassword = async () => {
    ...
    try {
      const response = await axios.post(`https://finmap.onrender.com/api/v1/resetPassword/${token}`, { password: newPassword,confirmPassword:confirmPassword });
      toast.success('Password reset successfully!');
      setTimeout(() => {
        navigate('employee-login'); // Redirect to the login page or homepage
      }, 3000);
    } catch (error) {
